...
test('checks if service is enabled', function () {
    ...
    (new GeneratePlaceholder($this->service))->handle(new AssetUploaded($asset, $asset->basename()));
    (new GeneratePlaceholder($this->service))->handle(new AssetReuploaded($asset, $asset->basename()));
});
...
test('generates no placeholders for unsupported assets', function () {
    ...
    (new GeneratePlaceholder($this->service))->handle(new AssetUploaded($txt, $txt->basename()));
    (new GeneratePlaceholder($this->service))->handle(new AssetReuploaded($svg, $svg->basename()));
});
